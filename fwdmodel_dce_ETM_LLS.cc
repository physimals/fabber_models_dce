/*  fwdmodel_DCE_ETM_LLS.cc - Implements a convolution based model for DCE analysis
...
FactoryRegistration<FwdModelFactory, DCE_ETM_LLS_FwdModel>
  DCE_ETM_LLS_FwdModel::registration("dce_ETM_LLS");

string DCE_ETM_LLS_FwdModel::ModelVersion() const
{
  return "$Id: fwdmodel_dce_ETM_LLS.cc,v 1.11 2016/01/06 15:20:47 Kallehauge Exp $";
}

void DCE_ETM_LLS_FwdModel::HardcodedInitialDists(MVNDist& prior,
    MVNDist& posterior) const
{
    Tracer_Plus tr("DCE_ETM_LLS_FwdModel::HardcodedInitialDists");
    ...
    // Fp or Ktrans whatever you belive
     ...
     if (Acq_tech != "none") {
     precisions(sig0_index(),sig0_index())=1e-2;
...
void DCE_ETM_LLS_FwdModel::Evaluate(const ColumnVector& params, ColumnVector& result) const
{
  Tracer_Plus tr("DCE_ETM_LLS_FwdModel::Evaluate");
   ...
   if (Acq_tech == "SRTF") {
        for (int i=1; i<=ntpts; i++){
            ...
            if (ctemp>0){
            //cout<<ctemp<<endl;
            Cvec(i)=((-1.0*log((1.0-exp(Tsat/T10))*(((data(i))/sig0)+(exp(Tsat/T10)/(1-exp(Tsat/T10))))))/(r1*Tsat));
            }else{
                //cout<<"Cvec(i)"<<endl;
                Cvec(i)=0.0;
            }
...
FwdModel* DCE_ETM_LLS_FwdModel::NewInstance()
{
  return new DCE_ETM_LLS_FwdModel();
}

void DCE_ETM_LLS_FwdModel::Initialize(ArgsType& args)
{
  Tracer_Plus tr("DCE_ETM_LLS_FwdModel::DCE_ETM_LLS_FwdModel");
...
vector<string> DCE_ETM_LLS_FwdModel::GetUsage() const
{
  vector<string> usage;

  usage.push_back( "\nThis model is the linear Extended Tofts model\n");
...
void DCE_ETM_LLS_FwdModel::DumpParameters(const ColumnVector& vec,
                                    const string& indent) const
{
    
}

void DCE_ETM_LLS_FwdModel::NameParams(vector<string>& names) const
{
  names.clear();
...
ColumnVector DCE_ETM_LLS_FwdModel::aifshift( const ColumnVector& aif, const float delta, const float hdelt ) const
{
...
void DCE_ETM_LLS_FwdModel::createconvmtx( LowerTriangularMatrix& A, const ColumnVector aifnew ) const
{
