...
FactoryRegistration<FwdModelFactory, DCEToftsFwdModel> 
  DCEToftsFwdModel::registration("dce");
...
static OptionSpec OPTIONS[] =
{
	...
	{ "aif", OPT_MATRIX, "Arterial signal image as ASCII matrix (will override supplemental data). Required, but can be given as 'none'", OPT_REQ, "" },
...
void DCEFwdModel::MakeParamIndex()
{
  ...
  for (int i=0; i<names.size(); i++) {
    if (names[i] == "Fp") fp_idx = i+1;
    if (names[i] == "Vp") vp_idx = i+1;
    if (names[i] == "PS") ps_idx = i+1;
    if (names[i] == "Ve") ve_idx = i+1;
    if (names[i] == "delay") delta_idx = i+1;
    if (names[i] == "sig0") sig0_idx = i+1;
    if (names[i] == "T10") t10_idx = i+1;
    if (names[i] == "Ktrans") ktrans_idx = i+1;
  }
}
...
void DCEToftsFwdModel::HardcodedInitialDists(MVNDist& prior, 
