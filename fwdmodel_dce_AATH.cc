/*  fwdmodel_dce_AATH.cc - An Adiabatic Approximation to the Tissue Homogeneity Model for Water Exchange in the Brain: I. Theoretical Derivation
...
void DCE_AATH_FwdModel::Initialize(FabberRunData &args)
{
...
void DCE_AATH_FwdModel::DumpParameters(const ColumnVector &vec, const string &indent) const
{
}
...
double DCE_AATH_FwdModel::LogOrNot(double p) const
{
    if (m_use_log)
        return log(p);
    else
...
ColumnVector DCE_AATH_FwdModel::aifshift(const ColumnVector &aif, const double delay) const
{
...
// Needs checking
double DCE_AATH_FwdModel::SignalFromConcentration(double C, double t10, double current_sig0) const
{
    double R10 = 1 / t10;
...
// Needs checking
double DCE_AATH_FwdModel::ConcentrationFromSignal(double s, double s0, double t10, double hct) const
{
...
ColumnVector DCE_AATH_FwdModel::compute_concentration(double Fp, double PS, double Vp, double Ve, const NEWMAT::ColumnVector &aif) const
{   
    if(Ve == 0) {
        Ve = 0.0001;
    }
